...
/// Computes the exact integer determinant of `basis`.
...
pub fn kernel_left(basis: &Matrix<i64>) -> Result<Matrix<i64>, DiophantineError> {
    let (h, u) = hnf_extended(basis)?;
...
pub fn kernel_right(basis: &Matrix<i64>) -> Result<Matrix<i64>, DiophantineError> {
    let t = transpose(basis);
    let res = kernel_left(&t)?;
    Ok(transpose(&res))
}
...
mod tests {
    ...
    fn matmul_dimensions() {
        ...
        assert!(matches!(
            matmul(&b, &b),
            Err(DiophantineError::InvalidDimensions(_))
        ));
    }
